...
def main():
    ...
    parser.add_argument("--log_odds", default=0.698, help="Restrict haplotype stitching to a specific chromosome.")
    # note: log_odds_ratio 0.698 is 5 times likely
...
def read_vcf(header_pop1_allele_table, header_pop2_allele_table, header_f1_allele_table):
    ...
    for record in stream_f1_vcf:
        ...
        for sample in record.samples:
            ...
            ### to do !! - retain the PG tag to recover the status and position of Indel to be phased downstream
            # - do this !!
...
####################  continue .... tomorrow ... introduce indel removal method .......
    ...
    ### write data in landscape vs. potrait mode .... - done
...
##################################################################################################33
    ...
    #### 03-B for population 01 vcf
    ## removing the phased and unphased indel from the df1
    # indel will have more than 3 characters (e.g: A/G in SNP, but AA/G is Indel)
    keys = [k for k in df1.columns.values if k.endswith('_PG_allele')]
    # this will make list of columns that ends with string '_PG_allele'

    for k in keys:
        df1.loc[df1[k].str.len() > 3, [k, k.replace('_PG_allele', '') + '_PI']] = '.'
        # any columns with matching columns names (with name+'_PG_allele' and name+'_PI') will be removed
    ...
    ## removing the phased indel from the df
    keys = [k for k in df2.columns.values if k.endswith('_PG_allele')]

    for k in keys:
        df2.loc[df2[k].str.len() > 3, [k, k.replace('_PG_allele', '') + '_PI']] = '.'
    ...
    #### 03-B for f1-hybrid vcf
    ## removing the phased indel from the df  - this can be removed to retain indels in Phasing
    keys = [k for k in df_f1.columns.values if k.endswith('_PG_allele')]

    for k in keys:
        df_f1.loc[df_f1[k].str.len() > 3, [k, k.replace('_PG_allele', '') + '_PI']] = '.'
    ...
    ## New try: retain the phase state from PG tag
    filter_col = [col for col in df_f1.columns.values if col.startswith(args.f1_sample)
                  and (col.endswith('_PG_allele') or col.endswith('_PI') or col.endswith('_PG'))]
    ...
    df_f1 = df_f1[filter_col].reset_index(level=['contig', 'pos', 'ref']) \
        .rename(columns={args.f1_sample + '_PG_allele': 'F1_' + args.f1_sample,
                         ...
                         args.f1_sample + '_PG': 'F1_' + args.f1_sample + '_PG'})
    ...
    # trying a new merge method - this was successful so keeping this

    df1_and_2 = pd.merge(df1, df2, on=['contig', 'pos'], how='outer', sort=True)
    df_merged = pd.merge(df_f1, df1_and_2, on=['contig', 'pos'], how='left', sort=True)


    ## Clear memory
    del df1, df2, df_f1, df1_and_2, filter_col



    ## select only the data (rows) where the phase_index (i.e _PI) value is integer (or say meaningful)
...
def markov_chain_process (stream_merged_data):
    ...
    cols = ['contig', 'pos', 'ref', 'F1_index', 'F1_x', 'F1_y', 'PG_x', 'PG_y' ]
    ...
    stream_csv['PG_x'], stream_csv['PG_y'] = zip(*stream_csv['F1_' + args.f1_sample + '_PG'].str.split('|'))
    ...
    ### remove several columns from the data frame
    # the data from these deleted columns already loaded into dictionary (haplotype_dict)
    # and these data except stream_csv['F1_sample_name'] will be used
    del stream_csv['F1_x']
    del stream_csv['F1_y']
    del stream_csv['ref']
    del stream_csv['PG_x']
    del stream_csv['PG_y']
    del stream_csv['F1_' + args.f1_sample + '_PG']
...
def count_haplotypes(df_chain, haplotype_dict):
    ...
    haplo_output.write('contig\tstart\tend\thaplotype_block\thap_X\thap_Y\tlog_odds_ratio\t' + args.pop1+'_hap\t' + args.pop2+ '_hap\n')
    ...
    stream_segregated_haplotype = 'contig\tpos\tref\thaplotype_block\thap_X\thap_Y\tlog_odds_ratio\t'\
                                  + args.pop1 + '_PG\t' + args.pop2 + '_PG\t' + args.pop1 + '_hap\t' + args.pop2 + '_hap'
    ...
    for k in haplotype_dict:
        ...
        #pos_range = '-'.join([pos.split('-')[0], pos.split('-')[-1]])
        start = pos.split('-')[0]; end = pos.split('-')[-1]

        ref = "-".join(haplotype_dict[k]['ref'])
        length = (len(haplotype_dict[k]['ref']))

        hap_X = "-".join(haplotype_dict[k]['F1_x'])
        hap_Y = "-".join(haplotype_dict[k]['F1_y'])
        allele_on_X = "-".join(haplotype_dict[k]['PG_x'])
        allele_on_Y = "-".join(haplotype_dict[k]['PG_y'])
        ...
        OddsRatio_hX_vs_hY_is_MyVsSp = log_Odds_Ratio = \
            math.log10((hapX_prob_My / hapX_prob_Sp) / (hapY_prob_My / hapY_prob_Sp))

        del OddsRatio_hX_vs_hY_is_MyVsSp

        if log_Odds_Ratio >= float(args.log_odds):
            My_haplotype = hap_X
            Sp_haplotype = hap_Y

            My_allele_config = allele_on_X
            Sp_allele_config = allele_on_Y
            print("the first haplotype %s is Mayodan" % str(hap_X))
            print(log_Odds_Ratio)

        elif log_Odds_Ratio <= -float(args.log_odds):
            My_haplotype = hap_Y
            Sp_haplotype = hap_X
            My_allele_config = allele_on_Y
            Sp_allele_config = allele_on_X
            print("the first haplotype %s is Spiterstulen" % str(hap_X))
            print(log_Odds_Ratio)

        else: # use a replace method to sub haplotype allele with Ns
            My_haplotype = '-'.join('N' *length)
            Sp_haplotype = '-'.join('N' *length)
            My_allele_config = '-'.join('N' *length)
            Sp_allele_config = '-'.join('N' *length)
            print("haplotype undefined")
            print(log_Odds_Ratio)
        ...
        output_hap.write(str(contig) + '\t' + start + '\t'+ end + '\t' + str(k) + '\t' + hap_X + '\t' + hap_Y + '\t' + str(
            log_Odds_Ratio) + '\t' + My_haplotype + '\t' + Sp_haplotype + '\n')


        # writing haplotype in potrait mode:
        stream_segregated_haplotype += '\n'+ str(contig) + '\t' + str(pos) +  '\t' + str(ref) + '\t' + str(k) + '\t' + hap_X + '\t' + hap_Y + \
                                       '\t' + str(log_Odds_Ratio) + '\t' + My_allele_config + '\t'+ Sp_allele_config + '\t' + My_haplotype + '\t' + Sp_haplotype
...
def haplotype_in_potrait(stream_segregated_haplotype):
    ...
    cols = ['pos', 'ref', 'hap_X', 'hap_Y', args.pop1 +'_hap', args.pop2 +'_hap', args.pop1 + '_PG', args.pop2 + '_PG']
    ...
    potrait_data = pd.DataFrame({"contig": np.repeat(df.contig.values, length),
        ...
        "log_odds_ratio": np.repeat(df.log_odds_ratio.values, length),
        args.pop1 + "_PG": list(chain.from_iterable(df[args.pop1 + '_PG'])),
        args.pop2 + "_PG": list(chain.from_iterable(df[args.pop2 + '_PG'])),
        args.pop1 + "_hap": list(chain.from_iterable(df[args.pop1 + '_hap'])),
        args.pop2 + "_hap": list(chain.from_iterable(df[args.pop2 + '_hap']))
    ...
    potrait_data.loc[s.duplicated(), 'log_odds_ratio'] = '-'

    ## clear memory
    del cols, length,


    potrait_data['contig'] = potrait_data['contig'].astype(int)
    potrait_data['pos'] = potrait_data['pos'].astype(int)
    potrait_data['log_odds_ratio'] = potrait_data['log_odds_ratio'].apply(lambda x: round(x, 3) if type(x) is float else x)

    potrait_data.sort_values(by=['contig', 'pos'], inplace=True, ascending=True)
